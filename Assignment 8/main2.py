...
class IntermediateCodeGenerator:
    ...
    def handle_assignment(self, lhs, expr):
        """ Handle assignment operation lhs = expr """
        result = self.postfix_to_quadruples(expr)
        # Assign the result to lhs
        self.quadruples.append(Quadruple('=', result, None, lhs))

    def display_quadruples(self):
        """ Display all the generated quadruples """
...
# Example usage
expression = input("Enter an infix expression (e.g., a=(b*(-c))+(b*(-c))): ")

# Handle assignment separately
if '=' in expression:
    lhs, rhs = expression.split('=')
    postfix_expr = infix_to_postfix(rhs)
    generator = IntermediateCodeGenerator()
    generator.postfix_to_quadruples(postfix_expr)
    generator.quadruples.append(Quadruple('=', generator.new_temp(), None, lhs))
else:
